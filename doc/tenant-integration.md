...
> - `test` 环境: 待部署后提供（阿里云 MySQL `pc-2zeycsbj0wa8951v2.rwlb.rds.aliyuncs.com:3306`）
...
| code | message | 说明 |
|------|---------|------|
| 400 | `instanceId 不能为空` | 未传 instanceId |
| 400 | `planId 不能为空` | 未传 planId |
| 401 | `缺少 X-Sign 签名` | 鉴权 Header 不完整 |
| 400 | `排行榜不存在或不属于该租户` | planId 错误或无权限 |
| 400 | `排行榜已终止或已过期，无法滚动` | 排行榜不在进行中状态 |
| 400 | `当前没有活跃实例，无需滚动` | 实例状态异常 |
| 400 | `实例已变更，请刷新后重试` | instanceId 与当前活跃实例不匹配，可能已有并发滚动 |
