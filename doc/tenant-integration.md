...
- 支持指标复用，同一个指标可关联多个排行榜（如"击杀数"同时用于日榜、周榜、月榜），上报一次自动更新所有匹配榜单
...
排行榜计划定义了一个排行榜的**配置规则**。指标与排行榜计划是松耦合的——同一个指标可被多个计划复用，上报时系统自动将数据更新到所有匹配的计划中：
...
> ```
> 
> **Shell 中计算签名**（文档 curl 示例使用 macOS 的 `shasum -a 256`，Linux 上请替换为 `sha256sum`）
...
- **始终检查响应的 `code` 字段**，而不是 HTTP 状态码 — 服务端所有响应 HTTP 状态码均为 200，业务结果通过 `code` 区分
- `code` 为 20000–29999 时表示鉴权或租户状态问题，检查凭据是否正确
- `code` 为 10001 等参数类错误时检查请求格式和必填字段
- `code` 为 10003 表示服务端内部异常，建议实现带退避的重试策略
...
### Q: 同一个指标关联了日榜、周榜、月榜，上报一次会更新哪些榜？

A: 全部更新。指标与排行榜是松耦合的，上报时系统自动匹配所有引用了该指标且处于活跃状态的排行榜计划，统一写入。
