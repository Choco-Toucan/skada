 ...
 * 排行榜滚动调度器
 * <p>
 * 负责周期性滚动策略的自动触发。使用分布式锁确保多节点下只有一个实例执行检查。
 * 实际的滚动和终止操作委托给 {@link LeaderboardConfigService}。
 * </p>
 */
@Component
public class LeaderboardRollScheduler {
    ...
    public LeaderboardRollScheduler(LeaderboardConfigService leaderboardConfigService,
    ...
    public void checkPeriodicRoll() {
        ...
        try {
            List<Leaderboard> allLeaderboards = leaderboardConfigService.findAll();
            ...
            for (Leaderboard lb : allLeaderboards) {
                ...
                if (lb.getEndTime() != null && now >= lb.getEndTime()) {
                    ...
                    try {
                        leaderboardConfigService.stop(lb.getId(), "scheduler");
                    } catch (Exception e) {
                        log.warn("自动终止排行榜失败: leaderboardId={}, {}", lb.getId(), e.getMessage());
                    }
                    continue;
                }
                ...
                if (now >= instanceEndTime) {
                    ...
                    try {
                        leaderboardConfigService.roll(lb.getId(), "scheduler");
                    } catch (Exception e) {
                        log.warn("自动滚动排行榜失败: leaderboardId={}, {}", lb.getId(), e.getMessage());
                    }
                }
            }
        } finally {
    ...
    long toMilliseconds(Integer value, String unit) {
